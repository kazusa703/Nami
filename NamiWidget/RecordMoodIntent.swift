...
struct RecordMoodIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult {
        ...
        let storedMin = WidgetConstants.sharedUserDefaults.integer(forKey: WidgetConstants.scoreRangeMinKey)
        let scoreRangeMin = storedMin != 0 ? storedMin : 1

        // source = "widget" で新規エントリを作成
        let entry = MoodEntry(score: score, maxScore: maxScore, scoreRangeMin: scoreRangeMin, source: "widget")
