...
struct NamiTimelineProvider: TimelineProvider {
    ...
    private func fetchEntry() -> MoodWidgetEntry {
        ...
        let minScore = {
            let stored = WidgetConstants.sharedUserDefaults.integer(forKey: WidgetConstants.scoreRangeMinKey)
            // scoreRangeMinKey が未設定(0)の場合、scoreRangeKey から判定
            if stored == 0 {
                let rangeKey = WidgetConstants.sharedUserDefaults.string(forKey: WidgetConstants.scoreRangeKey) ?? ""
