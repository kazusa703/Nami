...
struct CircularLockScreenView: View {
    ...
    var body: some View {
        if let score = entry.latestScore {
            ...
            let fraction = range > 0 ? Double(score - entry.minScore) / range : 0.5
...
struct RectangularLockScreenView: View {
    ...
    private var miniSparkline: some View {
        ...
        return Canvas { context, size in
            // 有効なスコアのみでスパークラインを描画
            let validScores = scores.filter { $0 > 0 || entry.minScore < 0 }
            guard validScores.count >= 2 else { return }

            let minVal = Double(entry.minScore)
            let maxVal = Double(entry.maxScore)
            let rangeVal = maxVal - minVal
            let points: [CGPoint] = scores.enumerated().compactMap { index, score in
                guard score > 0 || entry.minScore < 0 else { return nil }
                let x = size.width * Double(index) / 6.0
                let y = rangeVal > 0 ? size.height * (1 - (score - minVal) / rangeVal) : size.height * 0.5
