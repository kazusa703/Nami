...
struct MediumWidgetView: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            VStack(spacing: 8) {
                ...
                // インタラクティブスコアボタン（1行）
                MediumScoreButtons(minScore: entry.minScore, maxScore: entry.maxScore, theme: theme)
            }
            .padding(14)
        }
    }
    ...
    private func rightPanel(theme: WidgetTheme) -> some View {
        VStack(spacing: 6) {
            if let past = entry.randomPastRecord {
                // Past record memory section
                pastRecordSection(past: past, theme: theme)
            } else {
                // 週間平均バー
                if let avg = entry.weeklyAverage {
                    HStack {
                        Spacer()
                        Text(String(localized: "週間平均"))
                            .font(.system(size: 9, weight: .medium, design: .rounded))
                            .foregroundStyle(.secondary)
    ...
    // MARK: - Past record memory

    private func pastRecordSection(past: PastRecord, theme: WidgetTheme) -> some View {
                    ...
                    GeometryReader { geo in
                        VStack {
                            ...
                            if let daily, daily.entryCount > 0 {
                                let range = Double(entry.maxScore - entry.minScore)
                                let fraction = range > 0 ? (daily.averageScore - Double(entry.minScore)) / range : 0.5
                                let height = max(geo.size.height * CGFloat(fraction), 4)
                                let score = Int(daily.averageScore.rounded())

                                RoundedRectangle(cornerRadius: 3)
                                    .fill(theme.colorForScore(score, minScore: entry.minScore, maxScore: entry.maxScore))
                                    .frame(height: height)
                            } else {
                                // 記録なしの日: 薄いプレースホルダー
                                RoundedRectangle(cornerRadius: 3)
