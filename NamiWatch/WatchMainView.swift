    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                ...
                let storedMin = WatchConstants.sharedUserDefaults.integer(forKey: WatchConstants.scoreRangeMinKey)
                if storedMin == 0 {
                    // scoreRangeMinKey 未設定の場合、scoreRangeKey から判定
                    let rangeKey = WatchConstants.sharedUserDefaults.string(forKey: WatchConstants.scoreRangeKey) ?? ""
                    minScore = rangeKey.hasPrefix("neg") ? -maxScore : 1
                } else {
                    minScore = storedMin
                }

                selectedScore = Double(minScore + maxScore) / 2.0
            }
        }
    }
