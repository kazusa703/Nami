...
class StatsViewModel {
    ...
    func weeklyAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
        let startOfWeek = calendar.dateInterval(of: .weekOfYear, for: .now)?.start ?? .now
        let weekEntries = entries.filter { $0.createdAt >= startOfWeek }
        return normalizedAverage(of: weekEntries, scaleTo: currentMax)
    }

    /// 先週の平均スコアを計算する
    func lastWeekAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
    ...
    /// 今月の平均スコアを計算する
    func monthlyAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
        let startOfMonth = calendar.dateInterval(of: .month, for: .now)?.start ?? .now
        let monthEntries = entries.filter { $0.createdAt >= startOfMonth }
        return normalizedAverage(of: monthEntries, scaleTo: currentMax)
    }

    /// 先月の平均スコアを計算する
    func lastMonthAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
    ...
    /// 今年の平均スコアを計算する
    func yearlyAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
        let startOfYear = calendar.dateInterval(of: .year, for: .now)?.start ?? .now
        let yearEntries = entries.filter { $0.createdAt >= startOfYear }
        return normalizedAverage(of: yearEntries, scaleTo: currentMax)
    }
    ...
    func highestScore(entries: [MoodEntry], currentMax: Int = 10) -> (score: Int, date: Date)? {
        guard !entries.isEmpty else { return nil }
        // 正規化スコアが最大のエントリを探す
        let entry = entries.max(by: { $0.normalizedScore < $1.normalizedScore })!
        let scaled = Int(entry.scaledScore(to: currentMax).rounded())
        return (scaled, entry.createdAt)
    }

    /// 最低スコアとその記録日を返す（現在のレンジにスケール）
    func lowestScore(entries: [MoodEntry], currentMax: Int = 10) -> (score: Int, date: Date)? {
        guard !entries.isEmpty else { return nil }
    ...
    func mostCommonScore(entries: [MoodEntry], currentMax: Int = 10) -> Int? {
        guard !entries.isEmpty else { return nil }
        let distribution = scoreDistribution(entries: entries, maxScore: currentMax)
        return distribution.max(by: { $0.value < $1.value })?.key
    }

    /// スコアの分布（各スコアの記録回数）を返す
    /// 異なるmaxScoreのエントリは現在のレンジにスケーリングして集計する
    func scoreDistribution(entries: [MoodEntry], maxScore: Int = 10) -> [Int: Int] {
        var distribution: [Int: Int] = [:]
        for i in 1...maxScore { distribution[i] = 0 }
        for entry in entries {
            let scaled = entry.maxScore == maxScore
                ? entry.score
                : Int(entry.scaledScore(to: maxScore).rounded())
            let clamped = max(1, min(maxScore, scaled))
    ...
    func weekdayAverages(entries: [MoodEntry], currentMax: Int = 10) -> [Int: Double] {
        let calendar = Calendar.current
        ...
        for (weekday, group) in grouped {
            if let avg = normalizedAverage(of: group, scaleTo: currentMax) {
                result[weekday] = avg
            }
        }
        return result
    ...
    /// 時間帯別の平均スコアを返す
    func timeOfDayAverages(entries: [MoodEntry], currentMax: Int = 10) -> [TimeOfDay: Double] {
        let calendar = Calendar.current
    ...
    func weeklyRhythmData(entries: [MoodEntry], currentMax: Int = 10) -> [(label: String, index: Int, average: Double)] {
    ...
    func volatilityTrend(entries: [MoodEntry], currentMax: Int = 10) -> [(weekStart: Date, stdDev: Double)] {
        let calendar = Calendar.current
        ...
        for entry in sorted {
            ...
            let scaled = entry.normalizedScore * Double(currentMax - 1) + 1.0
    ...
    func tagAverageScores(entries: [MoodEntry], currentMax: Int = 10) -> [(tag: String, average: Double, count: Int)] {
        ...
        return grouped.compactMap { tag, tagEntries in
            guard let avg = normalizedAverage(of: tagEntries, scaleTo: currentMax) else { return nil }
    ...
    /// サンプル2件以上のタグのみ返す
    func nextDayEffect(entries: [MoodEntry], currentMax: Int = 10) -> [(tag: String, delta: Double, sampleSize: Int)] {
        let calendar = Calendar.current
        let sorted = entries.sorted { $0.createdAt < $1.createdAt }

        // 全体のベースライン（正規化平均）
        guard let baseline = normalizedAverage(of: sorted, scaleTo: currentMax) else { return [] }
        ...
        for day in allDays {
            ...
            guard let nextAvg = normalizedAverage(of: nextDayGroup, scaleTo: currentMax) else { continue }
    ...
    func tagImpactData(tag: String, entries: [MoodEntry], currentMax: Int)
        ...
        let withAvg = (withNorms.reduce(0, +) / Double(withNorms.count)) * Double(currentMax - 1) + 1.0
        let withoutAvg = (withoutNorms.reduce(0, +) / Double(withoutNorms.count)) * Double(currentMax - 1) + 1.0

        // エントリレベルの分布（チャート用）
        func buildDist(_ list: [MoodEntry]) -> [Int: Int] {
            var dist: [Int: Int] = [:]
            for e in list {
                let s = e.maxScore == currentMax ? e.score : Int(e.scaledScore(to: currentMax).rounded())
    ...
    func recordCountVsScore(entries: [MoodEntry], currentMax: Int)
        ...
        func dayAvg(_ days: [Date: [MoodEntry]]) -> Double {
            ...
            return n * Double(currentMax - 1) + 1.0
        }
    ...
    func tagUsageVsScore(entries: [MoodEntry], currentMax: Int)
        ...
        guard let tAvg = normalizedAverage(of: tagged, scaleTo: currentMax),
              let uAvg = normalizedAverage(of: untagged, scaleTo: currentMax) else { return nil }
    ...
    func weeklyReview(entries: [MoodEntry], currentMax: Int) -> WeeklyReview? {
        let calendar = Calendar.current
        guard let thisWeekStart = calendar.dateInterval(of: .weekOfYear, for: .now)?.start else { return nil }
        ...
        let average = avgNorm * Double(currentMax - 1) + 1.0
    ...
    private func generateWeeklySummary(entries: [MoodEntry], average: Double, currentMax: Int, previousAverage: Double?) -> String {
        let normAvg = (average - 1.0) / Double(currentMax - 1)
        let sorted = entries.sorted { $0.createdAt < $1.createdAt }
        ...
        if normAvg > 0.75 {
            ...
            if trendDelta > 0.1 {
                ...
                let prevNorm = (prev - 1.0) / Double(currentMax - 1)
    ...
    func reverseInsights(entries: [MoodEntry], currentMax: Int)
    ...
    /// 指定月の包括レポートを生成
    func monthlySummary(entries: [MoodEntry], currentMax: Int, month: Date) -> MonthlySummary? {
        let calendar = Calendar.current
    ...
    func tagChainPatterns(entries: [MoodEntry], currentMax: Int) -> [TagChain] {
        let calendar = Calendar.current
        let sorted = entries.sorted { $0.createdAt < $1.createdAt }
    ...
    func tagEchoEffect(entries: [MoodEntry], currentMax: Int) -> [TagEcho] {
        let calendar = Calendar.current
        let sorted = entries.sorted { $0.createdAt < $1.createdAt }
    ...
    func actionScoreDivergence(entries: [MoodEntry], currentMax: Int) -> [TagDivergence] {
        let calendar = Calendar.current
    ...
    func recoveryTriggers(entries: [MoodEntry], currentMax: Int) -> [RecoveryTrigger] {
        let calendar = Calendar.current
        let sorted = entries.sorted { $0.createdAt < $1.createdAt }
    ...
    func tagSynergyAnalysis(entries: [MoodEntry], currentMax: Int) -> [TagSynergy] {
        let taggedEntries = entries.filter { !$0.tags.isEmpty }
        ...
        return pairScores.compactMap { key, comboScores in
            ...
            let soloAvg1 = solo1Entries.reduce(0.0) { $0 + $1.normalizedScore * Double(currentMax - 1) + 1.0 } / Double(solo1Entries.count)
            let soloAvg2 = solo2Entries.reduce(0.0) { $0 + $1.normalizedScore * Double(currentMax - 1) + 1.0 } / Double(solo2Entries.count)
    ...
    /// 1年前の同じ週の平均スコア
    func sameWeekLastYearAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
    ...
    /// 1年前の同じ月の平均スコア
    func sameMonthLastYearAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
        let now = Date.now
    ...
    /// 去年全体の平均スコア
    func lastYearAverage(entries: [MoodEntry], currentMax: Int = 10) -> Double? {
        let calendar = Calendar.current
        let now = Date.now
    ...
    /// 過去の自分との比較データを生成
    func pastComparison(entries: [MoodEntry], currentMax: Int = 10) -> PastComparison {
        let lyWeek = sameWeekLastYearAverage(entries: entries, currentMax: currentMax)
        let lyMonth = sameMonthLastYearAverage(entries: entries, currentMax: currentMax)
        let lyYear = lastYearAverage(entries: entries, currentMax: currentMax)
        let hasData = lyWeek != nil || lyMonth != nil || lyYear != nil

        let curYear = yearlyAverage(entries: entries, currentMax: currentMax)
        ...
        return PastComparison(
            currentWeekAvg: weeklyAverage(entries: entries, currentMax: currentMax),
            lastYearSameWeekAvg: lyWeek,
            currentMonthAvg: monthlyAverage(entries: entries, currentMax: currentMax),
    ...
    private func normalizedAverage(of entries: [MoodEntry], scaleTo targetMax: Int) -> Double? {
        guard !entries.isEmpty else { return nil }
        let avgNormalized = entries.reduce(0.0) { $0 + $1.normalizedScore } / Double(entries.count)
        return avgNormalized * Double(targetMax - 1) + 1.0
    }
}
