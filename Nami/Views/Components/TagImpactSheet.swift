            ...
            HStack(spacing: 20) {
                ...
                VStack(spacing: 6) {
                    Text(String(format: "%.1f", impact.withAvg))
                        ...
                        .foregroundStyle(themeColors.color(for: Int(impact.withAvg.rounded()), minScore: currentMinScore, maxScore: currentMaxScore))
                    Text("タグあり")
                ...
                VStack(spacing: 6) {
                    Text(String(format: "%.1f", impact.withoutAvg))
                        .font(.system(size: 36, weight: .bold, design: .rounded))
                        .foregroundStyle(themeColors.color(for: Int(impact.withoutAvg.rounded()), minScore: currentMinScore, maxScore: currentMaxScore))
                    Text("タグなし")
                        .font(.system(.caption, design: .rounded, weight: .medium))
                        .foregroundStyle(.secondary)
